...
ggplot(rm_rcbd_df, aes(x = fdap, 
  ...
  facet_grid(. ~trtname)
```
we have no.of observations is four blocks per treatment
...
For interaction term (krate:nrate) and fdap we extract means and pairwise comparison
I will break the two way interaction (krate_kgha:nrate_kgha ) and main effect (fdap) at 5% significance level.

> What is significant here at alpha = 0.1?  
> Which means would you extract and perform pairwise comparison at this alpha level?  

break the three way interaction (krate_kgha:nrate_kgha:fdap) if we use 10% significance level.
...
ACF(mod1_default, resType = "n") %>% 
  plot(alpha = 0.01)  #setting 1% significance level
```
...
very positive (high) correlation at 0

bends helping us to interpret. 
If autocorrelation bar we see each one of the lags is within the bends, then the autocorrelation is not significant.
if crossing the bends and beyond them, autocorrelation is significant
default model, we see a significant autocorrelation at lag 1 and 2 not in 3
...
Compound symmetry is simplest and general is the most complex structure. With general, we are allowing the correlation structure of all those time points to change to different number (each of those pairs of those four harvest times can have their own correlation value)
compound symmetry saying i let you calculate a value, that value is going to be the same for all time points
...
interpret: lags 1 and 2 are properly modeled, lag three not
...
if this runs, probably will have six correlation values estimated and six intervals - how many intervals does not contain zero either lower and upper are negative and lower and upper are positive. if we have lower negative, upper positive then the interval contains zero.
one out of six correlation parameters didnt contain zero, so it is signioficant and other five contains zero
...
correlation and semivariogram goes in opposite directions.
points having low semivariogram are the ones that high very high correlation
...
In our case dist are time intervals
...
```
> Looking at the ANOVA table above, which means would you extract? three way interaction i.e krate_kgha:nrate_kgha:fdap 
...
plot showing each panel of the facet is one combination of nitrogen by potassium
we are comparing basically how a given treatment is increasing yield as we increase the harvest time
...
here(default model) three way interaction term is significant at 10% significance level but in general model three way interaction term is significant at 5% significant level.

degrees of freedom column in the default model is the df of the numerator
df getting impacted by estimating more parameters are those of the denominator of the F ratio

two way interaction term in default model is used to extract means and do pairwise comparison at 5% significant level
...
##Repeated measures does not impact the means, the means would be the same 
##Repeated measures is going to impact the error , error is important because having the model with proper error structure will change the P values of the ANOVA , change the error of the means, change the letter separation
